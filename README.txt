https://defiant-toad-ring.cyclic.app

Team Members 
Kian Afkhami (116690430)

App Description 
This is a simple website that allows users to place an order for an unspecified item. They can use their email to check their order and scan the QR Code to get back to their order page.

API links - Links to API(s) you are using.
https://www.qrtag.net/api/

YouTube Demo Video 
https://www.youtube.com/watch?v=kgTX_Jfe3Vo
Note: In the video I used an appliaction that can read QR codes from an image.
This is not required, you can use anything that works with a QR code such as a phone camera app.
